  ...
  methods: {
    ...
    login () {
      ...
      this.$refs.loginFromRef.validate(async valid => {
        ...
        const response = await this.$http.post(_this.$urls.LOGIN_URL, this.loginFrom)
        const data = response.data
        if (data.code === _this.$constant.SUCCESS_CODE) {
          const loginToken = data.data
          _this.$store.commit(_this.$constant.MUTATION_SET_LOGIN_TOKEN, loginToken)
          await _this.$router.push('/home')
        }
        _this.resetForm()
      })
    }
